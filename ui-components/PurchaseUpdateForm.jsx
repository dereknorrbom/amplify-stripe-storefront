...
export default function PurchaseUpdateForm(props) {
  ...
  const resetStateValues = () => {
    const cleanValues = purchaseRecord
      ? { ...initialValues, ...purchaseRecord }
      : initialValues;
    setBuyer(cleanValues.buyer);
    setBuyerEmail(cleanValues.buyerEmail);
    setAmount(cleanValues.amount);
    setFee(cleanValues.fee);
    setStripeChargeId(cleanValues.stripeChargeId);
    setOwner(cleanValues.owner);
    setErrors({});
  };
  const [purchaseRecord, setPurchaseRecord] = React.useState(purchaseModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getPurchase.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getPurchase
        : purchaseModelProp;
    ...
    queryData();
  }, [idProp, purchaseModelProp]);
  React.useEffect(resetStateValues, [purchaseRecord]);
  const validations = {
    buyer: [],
    buyerEmail: [],
    amount: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updatePurchase.replaceAll("__typename", ""),
            variables: {
              input: {
      ...
      {...getOverrideProps(overrides, "PurchaseUpdateForm")}
      {...rest}
    >
      <TextField
        label="Buyer"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || purchaseModelProp)}
